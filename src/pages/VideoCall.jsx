...
function VideoCall() {
  ...
  const clearMeeting = async () => {
    try {
      const res = await fetchWithAuth(
        `${import.meta.env.VITE_API_URI}/api/v1/mates/cancel-meeting/${mateId}`,
        {
          method: "DELETE",
          headers: {
  ...
  const handleIceCandidata = useCallback(
    async (event) => {
      if (event.candidate) {
      ...
      <div className="w-screen h-screen flex items-center justify-center bg-slate-900">
        {remoteSocketId ? (
          <div className="text-white text-center">
            ...
            <button
              className="bg-blue-600 px-4 py-2 mt-3 text-xl text-white rounded"
  ...
  return (
    <div className="flex justify-center w-screen h-screen bg-black overflow-hidden p-1 md:p-10">
      {expandedStream ? (
