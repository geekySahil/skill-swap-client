...
const MatchDetail = () => {
  ...
  const handleRequest = async (matchId, body) => {
    try {
      const res = await fetchWithAuth(
        ...
        }/api/v1/matches//update-match-status/${matchId}`,
        {
          method: "POST",
          headers: {
      ...
      socket.emit("notification", {
        ...
        to: mate.user._id,
        note: `${currentUser.username} has ${body.action} `,
      });
    } catch (error) {
  ...
  const handleAddReview = async (body) => {
    try {
      const res = await fetchWithAuth(
        `${import.meta.env.VITE_API_URI}/api/v1/feedback/add-feedback/${
          match.user._id
        }`,
        {
          method: "POST",
      ...
      socket.emit("notification", {
        ...
        from: currentUser._id,
        to: mate.user._id,
        note: `you've got feedback from ${currentUser.username} `,
      });
  ...
  const handleUpdateReview = async (reviewId, body) => {
    ...
    try {
      const res = await fetchWithAuth(
        `${
          import.meta.env.VITE_API_URI
        }/api/v1/feedback/update-feedback/${reviewId}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleDeleteReview = async (reviewId) => {
    try {
      const res = await fetchWithAuth(
        `${
          import.meta.env.VITE_API_URI
        }/api/v1/feedback/delete-feedback/${reviewId}`,
        {
          method: "DELETE",
          credentials: "include",
        },
  ...
  return (
    <div className="p-1 md:p-8 max-w-4xl mx-auto">
      ...
      <div className="border bg-white p-2 md:p-8 rounded-lg shadow-lg ">
        <div className="flex flex-col md:flex-row items-center space-x-6 my-6">
          <img
            src={
              match.user?.profilePicture || "https://via.placeholder.com/150"
            }
          ...
          <div className="text-lg mt-5">
            <p>
              <strong>Username:</strong> {match.user?.username}
            </p>
            <p>
              <strong>Email:</strong> {match.user?.email}
            </p>
            <p>
              <strong>Location:</strong> {match.user?.location}
            </p>
          </div>
        </div>
        <div className="text-lg mb-6">
          <p>
            ...
            {match.user.skills_collection.map((skill) => skill.name).join(", ")}
          </p>
          <p>
            <strong>Interests:</strong>{" "}
            {match.user.skills_wanted.map((skill) => skill.name).join(", ")}
          </p>
        </div>
        <div className="text-lg mb-6">
          <p>
            <strong>Availability:</strong>{" "}
            {match.user.availability || "Not specified"}
          </p>
        </div>
        <div className="text-sm md:text-lg mb-6">
          <span className="flex items-center">
            ...
            {feedbacks.length ? (
              renderStars(grossRating)
            ) : (
              <p className="text-yellow-500 font-bold mr-3">no ratings</p>
            )}
            
          </span>
          { feedbacks.lenght && `${ grossRating.toFixed(1)} out of 5`}
        </div>
        <div className="text-sm md:text-lg my-6">
          <p>
          ...
          {feedbacks && feedbacks.length > 0 ? (
            <>
              {!showAllReviews &&
                feedbacks.slice(0, 1).map((review, index) => (
                  <div
                    key={index}
                    className="my-4 p-4 border rounded-lg shadow-sm bg-gray-50"
                  >
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center">
                        <img
                          ...
                          src={review?.from_user_id?.profilePicture}
                          alt={review.from_user_id?.username}
                        />
                        <p className="text-gray-800">
                          <strong>{review?.from_user_id?.username}</strong>
                        </p>
                      </div>
        ...
        <div className="flex flex-col mb-6">
          ...
          <button
            onClick={() =>
              handleAddReview({ rating: newRating, review: newReview })
            }
            className={`bg-blue-500 text-white px-4 py-2 rounded shadow transition duration-300 ${
              !newRating || !newReview
        ...
        <div className="flex justify-end space-x-4">
          {match.yourStatus === "pending" && match.matchStatus === "pending" ? (
            <button
              onClick={() =>
                handleRequest(match._id, {
                  status: "accepted",
                  action: "requested You to become learning mates",
                })
              }
          ...
          ) : match.yourStatus === "accepted" &&
            match.matchStatus === "pending" ? (
            <button
              onClick={() =>
                handleRequest(match._id, {
                  status: "pending",
                  action: "reverted its request",
                })
              }
              className="bg-blue-600 text-white px-4 py-2 rounded shadow hover:bg-blue-700 transition duration-300"
            >
            ...
            <button
              onClick={() =>
                handleRequest(match._id, {
                  status: "pending",
                  action: "unblocked your request ",
                })
              }
              className="bg-red-900 text-white p-1 rounded mr-1"
            >
              Undo Decline
            </button>
          ) : null}
