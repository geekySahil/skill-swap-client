...
const Dashboard = () => {
  ...
  const { loading, error, currentUser, meetings } = useSelector(
    (state) => state.user
  );
  ...
  const filterData = () => {
    const filteredMatches = matches.filter(
      (match) =>
        match.matchStatus === "accepted" && match.yourStatus === "pending"
    );
  ...
  const handleRequest = async (match, body) => {
    try {
      const res = await fetchWithAuth(
        `${import.meta.env.VITE_API_URI}/api/v1/matches/update-match-status/${
          match._id
        }`,
        {
          method: "POST",
  ...
  const fetchMeetings = async () => {
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_URI}/api/v1/mates/get-all-meetings/${
          currentUser._id
        }`,
        {
          method: "GET",
  ...
  return (
    <div className="pt-20">
      ...
      <Element name="meetings" className="px-2">
                ...
                >
                  <div className="flex items-center space-x-4">
                    <img
                      ...
                      src={
                        meeting?.scheduledBy?._id === currentUser?._id
                          ? meeting?.scheduledWith?.profilePicture
                          : meeting?.scheduledBy?.profilePicture
                      }
                      alt={
                        meeting?.scheduledBy?._id === currentUser?._id
      ...
      <Element name="requests" className="px-2 mt-2">
        <div className="p-2 md:p-6 bg-white shadow-lg rounded-md min-h-screen overflow-y-auto">
                  ...
                  <div className="flex items-center justify-between space-x-4">
                    ...
                    <div className="flex flex-col">
                      <p className="text-md font-bold">
                        Request from {match?.user?.username}
                      </p>
                      <p className="text-sm text-gray">
                        {match?.user?.username} requested you for collab click
