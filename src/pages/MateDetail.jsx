...
const MateDetails = () => {
  ...
  const notificationsForUser = notifications?.filter(
    (notification) => notification.from !== mate.user._id
  )[0];
  ...
  const note = `Meeting scheduled at ${formattedTime(new Date())} by ${
  ...
  const getAllMeetings = async () => {
    try {
      const res = await fetchWithAuth(
        `${import.meta.env.VITE_API_URI}/api/v1/mates/meetings/${mateId}`,
        {
          method: "GET",
          headers: {
  ...
  const saveNotifications = async (message) => {
    try {
      const res = await fetchWithAuth(
        `${import.meta.env.VITE_API_URI}/api/v1/notifications/set-notification`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const onSetMeeting = async (data) => {
    ...
    try {
      const res = await fetchWithAuth(
        `${import.meta.env.VITE_API_URI}/api/v1/mates/set-meeting/${mate._id}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleJoinRoom = useCallback(
    (data) => {
      ...
      navigate(`/videoCall/${roomId}`, {
        state: { mateId: mate._id, meetingId: roomId },
      });
    },
    [navigate]
  );
  ...
  const cancelMeeting = async (meetingId) => {
    try {
      ...
      const res = await fetchWithAuth(
        `${import.meta.env.VITE_API_URI}/api/v1/mates/cancel-meeting/${
          mate._id
        }`,
        {
          method: "DELETE",
      ...
      dispatch(
        deleteMeetingFromState({ mateId: mateId, meetings: result.data })
      );
    } catch (error) {
      dispatch(failureForMate(error.message));
    }
  ...
  return (
    <div className="flex flex-col h-screen w-full mx-auto border rounded-lg shadow-lg bg-gray-100 p-1 md:p-4">
      <div className="flex flex-grow space-x-6">
        <div className="flex flex-col w-full bg-white rounded-lg shadow p-6 mb-6 overflow-y-auto">
          <h3 className="text-xl font-semibold mt-14 mb-4 text-slate-600">
            Scheduled Meetings
          </h3>
          {meetings?.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {meetings.map((meeting, index) => (
                <div
                  key={index}
                  className="flex flex-col mb-4 p-4 bg-gray-100 border-gray-400 rounded-lg"
                >
                  <p className="text-xl my-3">
                    Meeting Scheduled By{" "}
                    <span className="font-bold font-serif">
                      {meeting?.scheduledBy._id === currentUser?._id
                  ...
                  <div className="py-5 flex items-center space-x-3 font-bold">
                    <button
                      onClick={() => joinMeeting(meeting)}
                      className="flex items-center justify-center text-sm md:text-md  bg-violet-500 text-white px-4 py-2 rounded-lg shadow hover:bg-violet-700 focus:outline-none transition duration-200 ease-in-out"
                    >
                      <FaVideo />
                    </button>
                    <button
      ...
      {/* Action Buttons */}
      <div className="flex justify-around p-4 bg-slate-300 border-slate-700 rounded-lg shadow bottom-0 left-0 right-0">
        <button
          onClick={onChat}
